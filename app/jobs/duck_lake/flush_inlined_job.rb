...
module DuckLake
  ...
  # periodic flush the catalog Postgres grows unbounded and writes get slow.
  #
  ...
  # worker is in an OOM-restart loop. Disabling flush lets UnifiedConsumer
  # keep draining the ingest queue (writes go inline, fine in the short
  # term); re-enable once you can give the worker enough RAM to drain the
  # one-off backlog. Distinct from DUCKLAKE_DISABLED, which no-ops all
  ...
  class FlushInlinedJob
    ...
    def flush_table(table)
      ...
      result.sum { |row| row["rows_flushed"].to_i }
    end

    # Returns { "events" => 12345, "transactions" => 0, ... }. The DuckLake
    # catalog (Postgres) holds a per-table inlined-rows table whose name is
    ...
    # count on each. count(*) on a few-MB Postgres table is cheap.
    def inlined_counts_by_table
      ...
      mapping = ApplicationDucklakeRecord.query(<<~SQL)
        ...
        FROM __ducklake_metadata_splat_lake.ducklake_inlined_data_tables idt
        JOIN __ducklake_metadata_splat_lake.ducklake_table t
      ...
      mapping.each do |row|
        ...
        result = ApplicationDucklakeRecord.query(
          "SELECT count(*) AS c FROM __ducklake_metadata_splat_lake.#{inlined}"
        )
